...
def eqWall (al ,alN ,vm ,nu ,mu ,psiN, vw, rho, solution = -1):
     ...
     r=ga2p/ga2m*(1-rho*abs(ga2p)**.5*vp/(w_from_alpha (al ,alN ,nu ,mu)+1e-100))**2 #r=(T_m/T_p) at rarefraction front 
     """
     if verb and verb1:
         verb=False
         verb1=False
         print(r'$\Delta S= $',rho*abs(ga2p)**.5*vp/(ga2vw**.5*w_from_alpha (al ,alN ,nu ,mu)+1e-100))
     """
